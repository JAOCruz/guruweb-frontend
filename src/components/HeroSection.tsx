  ...
  useEffect(() => {
    scene.traverse((child: any) => {
      if (child.isMesh && child.material) {
        ...
        if (child.material.emissive) child.material.emissive.set("#000044");
  ...
  return (
    <div className="relative flex h-screen w-full flex-col overflow-hidden bg-[#020617] font-sans selection:bg-blue-500/30">
      ...
      <motion.nav
        ...
        className={`fixed top-0 left-0 z-50 w-full px-6 py-6 transition-all duration-300 ${
          scrolled ? "border-b-4 border-[#000080] bg-[#020617]" : ""
        }`}
      >
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          {/* Logo PNG + text top-left */}
      ...
      >
        {/* 3D Balance — centered above heading */}
        <motion.div
          ...
          >
            <motion.span
              ...
              style={{
                ...
                textShadow: `${2 + mousePosition.x * 4}px ${
                  2 + mousePosition.y * 4
                }px 0 #0000FF, ${4 + mousePosition.x * 8}px ${
                  4 + mousePosition.y * 8
                }px 0 #0000AA, ${6 + mousePosition.x * 12}px ${
                  6 + mousePosition.y * 12
                }px 0 #000080, ${8 + mousePosition.x * 16}px ${
                  8 + mousePosition.y * 16
                }px 0 #000050`,
              }}
            >
              Una Experiencia
        ...
        {/* Subtitle Tag — BIGGER */}
        <motion.div
          className="mb-8 inline-block md:mb-10"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ delay: 0.5, duration: 1 }}
        >
          <div className="border-3 border-[#000080] bg-[#0000FF] px-4 py-2 text-sm font-black uppercase tracking-widest text-white shadow-[4px_4px_0px_0px_rgba(0,0,128,1)] md:px-6 md:py-2.5 md:text-base lg:text-lg">
        ...
        >
          ...
          >
            ...
            <ChevronRight
              size={20}
              className="text-blue-200 transition-transform group-hover:translate-x-1"
            />
          </button>
        </motion.div>
        ...
        <motion.div
          className="mt-6 inline-flex items-center gap-2 border-2 border-[#000080] bg-black px-3 py-2 text-xs font-black uppercase tracking-widest text-white shadow-[5px_5px_0px_0px_rgba(0,0,128,1)] md:mt-8 md:px-4 md:py-2.5 md:text-sm"
          animate={{ y: [0, -10, 0] }}
